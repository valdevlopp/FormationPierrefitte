...
function envoi() {
    ...
    if (document.getElementById('prenom').value &&
        ...
        $('#formulaire_user').hide();
        // clignotement
        for (i = 0; i < 4; i++) {
                // first selectionne le 1er #list_users .card
                // change son opacite pour creer effet clignotant
                $('#list_users .card').first().fadeTo(1000, 0.5).fadeTo('fast', 1.0);
        }
    }
...
function afficherUtilisateurs() {
    ...
    // remplacer par jQuery $.each(myUsers, function(){
    //
    // });
...
$(function() {
  $('#formulaire_user').hide();
  $('#show-user-form-btn').bind('click', function() {
    $('#formulaire_user').toggle();
  });
  // CODE DE MEHDI POUR FONCTION RECHERCHE
